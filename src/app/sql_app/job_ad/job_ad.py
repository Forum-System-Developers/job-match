...
class JobAd(Base):
    """
    Represents a job advertisement in the job matching application.

    Attributes:
        id (uuid.UUID): Unique identifier for the job advertisement.
        company_id (uuid.UUID): Foreign key referencing the company offering the job.
    ...
    Relationships:
        job_ads_requirements (list[JobAdsRequirement]): List of requirements for the job advertisement.
        category (Category): Relationship to the Category model.
        location (City): Relationship to the City model.
